...
export interface RoleInfo {
  role: Role;
  label: string;
  /** One-sentence summary suitable for a native `title=` tooltip. */
  short: string;
  /** Bullet list for the legend card. */
  bullets: string[];
}

export const ROLE_DESCRIPTIONS: Record<Role, RoleInfo> = {
  requester: {
    role: "requester",
    label: "Requester",
    short:
      "Submits intake requests. Sees only their own requests. No review or admin access.",
    bullets: [
  ...
  reviewer: {
    ...
    label: "Reviewer",
    short:
      "Reviews and decides on submitted requests. Can comment on any request.",
    bullets: [
  ...
  architect: {
    ...
    label: "Architect",
    short:
      "Same permissions as Reviewer today. Reserved for future architecture sign-off.",
    bullets: [
...
export const ROLE_ORDER: Role[] = ["requester", "reviewer", "architect", "admin"];

/** Multi-line summary for a combined tooltip on a label/header. */
export const ALL_ROLES_TOOLTIP = ROLE_ORDER.map(
  (r) => `${ROLE_DESCRIPTIONS[r].label}: ${ROLE_DESCRIPTIONS[r].short}`,
).join("\n\n");

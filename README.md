...
| `VALIDATION_RUNNER_IMAGE` | Slim runner image for the validation uploader sidecar + inline scripts. Set via `validation.runnerImage`. Empty → log-only validation. |
| `VALIDATION_INGEST_BASE_URL` | Foyre URL reachable from validation pods, for evidence upload. Set via `validation.ingestBaseUrl`. Empty → log-only validation. |
| `VALIDATION_ALLOW_INLINE_SCRIPTS` | Allow `custom.script` steps (default true). Set via `validation.allowInlineScripts`. |
